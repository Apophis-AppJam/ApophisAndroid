...
class SeventhDayChatActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        seventh_btn_back.setOnClickListener { onBackPressed() }
        ...
        seventhUserChatAdapter.setOnItemClickListener(object :
            SeventhChatAdapter.OnItemClickListener {
        ...
        seventhUserChatAdapter.setOnRandomCoinListener(object :
            SeventhChatAdapter.OnRandomCoinListener {
        ...
        seventhUserChatAdapter.setOnDelayClickListener(object :
            SeventhChatAdapter.OnDelayClickListener {
                    ...
                    if (response.isSuccessful) {
                        if (response.body()!!.success) {
                            ...
                            for (i in response.body()!!.data.chat.indices) {
                                ...
                                if (nextAction == "음성송출3") {
                                    Handler().postDelayed({
                                        var seventhThirdPlayer = MediaPlayer.create(
                                            this@SeventhDayChatActivity,
                                            R.raw.seventhday_3
                                        )
                                ...
                                if (nextAction == "음성송출4") {
                                    Handler().postDelayed({
                                        var seventhFourthPlayer = MediaPlayer.create(
                                            this@SeventhDayChatActivity,
                                            R.raw.seventhday_4
                                        )
...
//
                                    Handler().postDelayed({
                                        seventh_videoView.isInvisible = true
                                        seventh_lottie_final.isGone = true
                                    }, 16000)
                                }
                            ...
                            when (tag) {
                                0, 1, 2, 3, 5 -> {
                                    ...
                                    seventh_btn_chat_send.setOnClickListener {
                                        ...
                                        seventh_rcv_chat.smoothScrollToPosition(
                                            seventhUserChatAdapter.itemCount - 1
                                        )
                                ...
                                7 -> {
                                    seventh_et_chat_message.hint = "텍스트를 입력하세요"
                                    seventh_btn_chat_send.isEnabled = true
                                    seventh_btn_chat_send.setOnClickListener {
                                        val userChoice = seventh_et_chat_message.text.toString()
                                        val chatRight =
                                        ...
                                        Log.d("다다 reply로 보내는 idx", chatDetailsIdx.toString())
                                        postReplyToServer(jwt, chatDetailsIdx, 1, userChoice)
                                        seventh_et_chat_message.hint = "선택지를 골라주세요"
                                    }
                                }
                                9 -> {
                                    Handler().postDelayed({
                                        val intent = Intent(
                                            this@SeventhDayChatActivity,
                                            SeventhDayTarotActivity::class.java
                                        )
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        if (response.body()!!.success) {
                            ...
                            if (randomCoin == "0") {
                                getAponymousChatFromServer(jwt, chatDetailsIdx + 2)
                            } else {
